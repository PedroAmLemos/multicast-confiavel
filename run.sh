...
if [ -z "$TMUX" ]; then
    ...
    tmux new-window -t multicast -n 'multicast_window' './multicast fred hosts'

    # Split the new window vertically and run the second command
    tmux split-window -t multicast:multicast_window -h './multicast rubens hosts'

    # Split the pane again (from the first pane) and run the third command
    tmux split-window -t multicast:multicast_window -h './multicast carlos hosts'
    ...
    tmux new-window -n 'multicast_window' './multicast fred hosts'

    # Split the new window vertically and run the second command
    tmux split-window -h './multicast rubens hosts'

    # Split the pane again (from the first pane) and run the third command
    tmux split-window -h './multicast carlos hosts'
